...
#if defined(NDEBUG)
# error "Uidd cannot be compiled without assertions."
#endif
...
std::unique_ptr<UiddState> globalState;
...
const std::string strMessageMagic = "Uidd Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            UiddDGP uiddDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = uiddDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = uiddDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            ...
            UiddTxConverter converter(tx, NULL, NULL, contractflags);
            ExtractUiddTX resultConverter;
            if(!converter.extractionUiddTransactions(resultConverter)){
            ...
            std::vector<UiddTransaction> uiddTransactions = resultConverter.first;
            std::vector<EthTransactionParams> uiddETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(UiddTransaction uiddTransaction : uiddTransactions){
                sumGas += uiddTransaction.gas() * uiddTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, uiddTransaction.gasPrice());
                } else {
                    txMinGasPrice = uiddTransaction.gasPrice();
                }
                VersionVM v = uiddTransaction.getVersion();
                if(v.format!=0)
                ...
                if(uiddTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(uiddTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(uiddTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)uiddTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(uiddETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > uiddTransactions.size())
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s UIDD additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // uidd
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // uidd
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    UiddTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<UiddTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path uiddDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(uiddDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(uiddDir.string(), std::ios::in | std::ios::out);
...
bool UiddTxConverter::extractionUiddTransactions(ExtractUiddTX& uiddtx){
    ...
    std::vector<UiddTransaction> resultTX;
    ...
    uiddtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool UiddTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool UiddTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
UiddTransaction UiddTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    UiddTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = UiddTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = UiddTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t UiddTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // uidd
    UiddDGP uiddDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setUiddSchedule(uiddDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = uiddDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = uiddDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = uiddDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // uidd
...
///////////////////////////////////////////////////////////////////////////////////////// uidd
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            UiddTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractUiddTX resultConvertUiddTX;
            if(!convert.extractionUiddTransactions(resultConvertUiddTX)){
            ...
            if(!CheckMinGasPrice(resultConvertUiddTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertUiddTX.first, blockGasLimit, pindex->pprev);
            ...
            for(UiddTransaction& qtx : resultConvertUiddTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertUiddTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertUiddTX.first[k].from(),
                        resultConvertUiddTX.first[k].to(),
                        ...
                        resultConvertUiddTX.first[k].getNVout()
                    });
                }
...
////////////////////////////////////////////////////////////////// // uidd
    ...
    if(pindex->nHeight > 5000 && !Params().MineBlocksOnDemand()) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(1038950000000ULL + ((uint64_t)(pindex->nHeight - 5000) * 20000ULL)) * COIN){
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // uidd
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // uidd
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // uidd
            globalState->setRootUTXO(oldHashUTXORoot); // uidd
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // uidd
        uint32_t sizeBlockDGP = uiddDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Uidd PoS we need to sync this after every block to ensure txdb is populated for
