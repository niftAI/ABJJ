...
void UiddDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool UiddDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
uint64_t UiddDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
bool UiddDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void UiddDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void UiddDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void UiddDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void UiddDGP::createParamsInstance(){
...
dev::Address UiddDGP::getAddressForBlock(unsigned int blockHeight){
...
void UiddDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void UiddDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void UiddDGP::parseStorageOneUint64(uint64_t& value){
...
void UiddDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule UiddDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
