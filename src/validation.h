...
using ExtractUiddTX = std::pair<std::vector<UiddTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const CAmount DEFAULT_GAS_PRICE=0.000040*COIN;
static const CAmount MAX_RPC_GAS_PRICE=0.000100*COIN;

static const size_t MAX_CONTRACT_VOUTS = 1000; // uidd
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashUiddRoot,
    const dev::h256& oldHashStateRoot, const std::vector<UiddTransaction>& transactions);
...
class UiddTxConverter{

public:

    UiddTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL, unsigned int flags = SCRIPT_EXEC_BYTE_CODE) : txBit(tx), view(v), blockTransactions(blockTxs), sender(false), nFlags(flags){}

    bool extractionUiddTransactions(ExtractUiddTX& uiddTx);
...
private:
    ...
    UiddTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<UiddTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
...
private:
    ...
    std::vector<UiddTransaction> txs;
